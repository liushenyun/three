  ...
  <div class="device-manage-outer">
    <!-- top Satrt -->
      <div class="dt-search-top">
        <!-- 租赁模式 -->
        <dt-select-option ref='rentModelSearchRef' :optiosTitle='$ElSelectName.rentModelHasAll.title' :optionsName ='$ElSelectName.rentModelHasAll.name' :conWidth = '$ElSelectName.rentModelHasAll.width'></dt-select-option>
        <!-- 规则名称 -->
        <dt-select-option-dynamic ref='templateSearchRef' optiosTitle='规则名称' :optionsData ='rulesDefault' :conWidth = '$ElSelectName.choiceTemplate.width'></dt-select-option-dynamic>
          ...
          <table class="dt-table-same" cellspacing="0" cellpadding="0">
                <thead>
                  <tr>
                    ...
                    <th class="tw-device-id">租赁规则</th>
                    <th class="tw-device-id">租赁类型</th>
              ...
              <tbody>
                <tr v-for="(item,index) in listArr" :key="index">
                  ...
                  <td>一年期</td>
                  <td>一年期</td>
                  <!-- <td class="dt-table-edit"><b>重置</b><b @click="tableEditActive(item)">编辑</b></td> -->
                </tr>
              </tbody>
          </table>
...
<script>
import { dtSlidePage, dtSelectOptionDynamic } from '../../global/searchComponents';
...
export default {
  ...
  data() {
    return {
      ...
      slideAreaData: [0, 0, 0],
      slideAreaDefault: ['', '', ''],
      // rulesDefault: [{ value: '', label: '' }]
      rulesDefault: [{ value: '1', label: '一年期' }, { value: '2', label: '二年期' }]
    }
  },
  mounted() {
  ...
  methods: {
    getUrlData() {
      return {
        ...
        'lease_type': this.$refs.rentModelSearchRef.value,
        ...
        'lease_name': this.$refs.templateSearchRef.value
      }
    },
    getRefs() {
      let _refsValue = {
    ...
    getMainList(params) {
      ...
      this.$http.deviceLeaseSetList(params, msg => {
        let _msg = msg.data;
